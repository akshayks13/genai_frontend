          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold text-grey-900">Overall Progress</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                {roadmapData.phases.map((phase, index) => (
                  <div key={phase.id} className="text-center">
                    ...
                    <div className="text-xs font-medium text-grey-900">{phase.title}</div>
                    <div className="text-xs text-grey-600">{phase.duration}</div>
                  </div>
                ))}
              </div>
            </CardContent>
            ...
            >
              {roadmapData.phases.map((phase, phaseIndex) => (
                <Card key={phase.id} className="overflow-hidden">
                  <CardHeader 
                    className={`cursor-pointer ${phase.status === 'completed' ? 'bg-green-50' :
                      phase.status === 'in-progress' ? 'bg-blue-50' : 'bg-grey-50'}`}
                    ...
                    {phase.progress > 0 && (
                      <div className="w-full bg-grey-200 rounded-full h-2 mt-3">
                        <motion.div 
                          ...
                          transition={{ duration: 1, delay: phaseIndex * 0.1 }}
                        />
                      </div>
                    )}
                  </CardHeader>
                      ...
                      <CardContent className="pt-0">
                        <div className="space-y-3">
                          {phase.milestones.map((milestone) => (
                            <div key={milestone.id} className="flex items-center justify-between p-3 border border-grey-200 rounded-lg">
                              <div className="flex items-center space-x-3">
                                {getStatusIcon(milestone.status)}
                                <div>
                                  <h4 className="font-medium text-grey-900">{milestone.title}</h4>
                              ...
                              <Button variant="outlined" size="sm">
                                {milestone.status === 'completed' ? 'Review' : 
            ...
            >
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Your immediate focus areas</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-3">
                ...
                <CardContent>
                  <div className="space-y-3">
                    {certifications.map((cert, index) => (
                      <div key={index} className="border border-grey-200 rounded-lg p-3">
                        <h4 className="font-medium text-grey-900 mb-1">{cert.name}</h4>
                        <p className="text-sm text-grey-600 mb-2">{cert.provider}</p>
                        <div className="flex justify-between items-center text-xs">
                          <span className="text-grey-500">{cert.duration}</span>
                          <span className={`px-2 py-1 rounded-full ${
                            cert.priority === 'Recommended' ? 'bg-green-100 text-green-700' :
            ...
            >
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Optimize your learning time</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-grey-600">Daily Goal</span>
                      <span className="font-medium">2 hours</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-grey-600">This Week</span>
                      <span className="font-medium text-green-600">12/14 hours</span>
                    </div>
                    <div className="flex justify-between items-center">
